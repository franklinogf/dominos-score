...
export const gamesTable = sqliteTable('games', {
  ...
  gameSize: integer({ mode: 'number' }).notNull(),
  winningLimit: integer({ mode: 'number' }).notNull(),
...
export const roundsTable = sqliteTable('rounds', {
  id: integer({ mode: 'number' }).primaryKey({ autoIncrement: true }),
  gameId: integer({ mode: 'number' }).notNull(),
  roundWinnerId: integer({ mode: 'number' }),
});

export const playersTable = sqliteTable('players', {
  id: integer({ mode: 'number' }).primaryKey({ autoIncrement: true }),
  roundId: integer({ mode: 'number' }).notNull(),
  name: text().notNull(),
